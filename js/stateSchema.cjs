...
const STATE_SCHEMA_NAME = 'pm-ops-map-state';

function buildStatePayload({ company, departments, team, workOrders, portfolio, exportedAt = new Date().toISOString() }) {
  return {
...
function validateImportedState(data, orgData) {
  const report = {
    ok: true,
    errors: [],
    warnings: [],
  ...
  if (!data || typeof data !== 'object') report.errors.push('File is not a JSON object.');
  if (!Array.isArray(data?.departments)) report.errors.push('Missing departments array.');
  if (data?.schema && data.schema !== STATE_SCHEMA_NAME) report.warnings.push(`Unexpected schema "${data.schema}". PM Ops Map will import compatible department data only.`);
  if (Number(data?.schemaVersion || 1) > STATE_SCHEMA_VERSION) report.warnings.push(`This file was exported by a newer schema version (${data.schemaVersion}). Unknown fields will be ignored.`);
  ...
  data.departments.forEach(savedDept => {
    ...
    savedDept.tasks.forEach(savedTask => {
      ...
      report.matchedTasks++;
      if (savedTask.dueDate && !isValidISODateValue(savedTask.dueDate)) report.invalidDueDates++;
    });
  });

  if (report.invalidDueDates) report.warnings.push(`${report.invalidDueDates} invalid due date value${report.invalidDueDates === 1 ? '' : 's'} will be cleared.`);
  if (report.skippedDepartments || report.skippedTasks) report.warnings.push(`${report.skippedDepartments} department${report.skippedDepartments === 1 ? '' : 's'} and ${report.skippedTasks} task${report.skippedTasks === 1 ? '' : 's'} do not match this app version.`);
  if (report.matchedTasks === 0) report.errors.push('No matching tasks were found for this app version.');
